...
parser.add_argument('-m', dest = 'maf', type = float, required=True,  help = 'minimum minor allele frequency to consider fixed')
...
for line_gfile, line_snpdet in izip(open(args.gfile), open(args.snpdet)):
	...
	if g1_AF < args.maf and g2_AF > (1-args.maf):
		output.write(scafnum+'\t'+ pos +'\t'+ref_allele+'\t'+alt_allele+'\t'+ID+'\n')
	elif g1_AF > (1-args.maf) and g2_AF < args.maf:
		output.write(scafnum+'\t'+ pos +'\t'+ref_allele+'\t'+alt_allele+'\t'+ID+'\n')
	else:
		pass
